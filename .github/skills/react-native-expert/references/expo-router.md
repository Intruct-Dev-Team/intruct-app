...
Note: apply the same guard pattern to your tab layout (check `isLoading`, redirect when `!user`) and then render your real `<Tabs ... />` config.

## Deep Linking

Add a linking config so external URLs map to routes.
...
- `myapp://details/123` -> `app/details/[id].tsx`
- `https://yourdomain.com/details/123` -> `app/details/[id].tsx`
...
| `back()`       | Go back         |
| `dismissAll()` | Dismiss modals  | # Expo Router |
