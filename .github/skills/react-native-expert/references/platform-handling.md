...
## Safe Area (recommended)

Prefer `react-native-safe-area-context` for consistent behavior across devices.
...
// More control: useSafeAreaInsets
...
- The built-in `SafeAreaView` from `react-native` is acceptable for very simple layouts or legacy projects.
- Use `SafeAreaProvider` / `useSafeAreaInsets` when you need full control and reliable behavior across platforms.
...
| `BackHandler`          | Android back button | # Platform Handling |
